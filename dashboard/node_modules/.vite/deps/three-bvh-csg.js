...
// node_modules/three-bvh-csg/src/libs/cdt2d.js
...
var require_orientation = __commonJS({
  "node_modules/robust-orientation/orientation.js"(exports, module) {
    ...
    var ERRBOUND3 = (3 + 16 * EPSILON3) * EPSILON3;
    var ERRBOUND4 = (7 + 56 * EPSILON3) * EPSILON3;
...
var cdt2d_default = require_cdt2d();

// node_modules/three-bvh-csg/src/core/utils/Pool.js
var Pool = class {
...
function edgesToIndices(edges, outputVertices, outputIndices, epsilonScale) {
  ...
  for (let i = 0, l = edges.length; i < l; i++) {
    ...
    for (let i1 = i + 1; i1 < l; i1++) {
      ...
      const dist = edge0.distanceSqToLine3(edge1, _vec, _vec22);
...
// node_modules/three-bvh-csg/src/core/LegacyTriangleSplitter.js
var EPSILON2 = 1e-10;
...
var IntersectionMap = class {
  ...
  add(id, intersectionId, coplanar = false) {
    ...
    if (coplanar) {
      if (!coplanarSet.has(id)) {
        coplanarSet.set(id, /* @__PURE__ */ new Set());
      }
  ...
  addIntersectionEdge(id, edge) {
    const { edgeSet } = this;
    if (!edgeSet.has(id)) {
      edgeSet.set(id, /* @__PURE__ */ new Set());
    }
...
// node_modules/three-bvh-csg/src/core/utils/intersectionUtils.js
...
function isTriangleCoplanar(triA, triB) {
  ...
  if (Math.abs(1 - Math.abs(dot)) >= COPLANAR_NORMAL_EPSILON) {
    return false;
  }
...
function getHitSide(tri, bvh, matrix = null) {
  ...
  if (matrix) {
    _ray2.origin.applyMatrix4(matrix);
    _ray2.direction.transformDirection(matrix);
  }
  ...
  a.geometry.boundsTree.bvhcast(b.geometry.boundsTree, _matrix, {
    intersectsTriangles(triangleA, triangleB, ia, ib) {
      if (!isTriDegenerate(triangleA) && !isTriDegenerate(triangleB)) {
        ...
        const intersected = isCoplanarIntersection || triangleA.intersectsTriangle(triangleB, _edge2, true);
        if (intersected) {
          ...
          if (isCoplanarIntersection) {
            ...
            const ea = _edgePool.getInstance().copy(_edge2);
            const eb = _edgePool.getInstance().copy(_edge2);
...
function performSplitTriangleOperations(a, b, intersectionMap, operations, invert, splitter, builders, groupOffset = 0) {
  ...
  _inverseMatrix.copy(_matrix2).invert();
  if (invert) {
    _builderMatrix.copy(_matrix2);
  } else {
  ...
  for (let i = 0, l = splitIds.length; i < l; i++) {
    ...
    if (invert) {
      _triA.a.applyMatrix4(_matrix2);
      _triA.b.applyMatrix4(_matrix2);
      _triA.c.applyMatrix4(_matrix2);
    }
    splitter.reset();
    ...
    for (let i2 = 0, l2 = builders.length; i2 < l2; i2++) {
      builders[i2].initInterpolatedAttributeData(a.geometry, _builderMatrix, _normalMatrix, ia0, ia1, ia2);
    }
    _traversed.clear();
    for (let ib = 0, l2 = triangles.length; ib < l2; ib++) {
      ...
      const raycastMatrix = invert ? null : _matrix2;
      let hitSide = null;
      ...
      if (_builders.length !== 0) {
        ...
        while (stack.length > 0) {
          ...
          for (let k = 0, lk = _builders.length; k < lk; k++) {
            ...
            const invert2 = invertedGeometry !== invertTri;
            builder.appendInterpolatedAttributeData(groupIndex, _barycoordTri.a, t0, invert2);
            if (invert2) {
              builder.appendInterpolatedAttributeData(groupIndex, _barycoordTri.c, t2, invert2);
              builder.appendInterpolatedAttributeData(groupIndex, _barycoordTri.b, t1, invert2);
            } else {
              builder.appendInterpolatedAttributeData(groupIndex, _barycoordTri.b, t1, invert2);
              builder.appendInterpolatedAttributeData(groupIndex, _barycoordTri.c, t2, invert2);
            }
...
function performWholeTriangleOperations(a, b, splitTriSet, operations, invert, builders, groupOffset = 0) {
  ...
  for (let id = 0; id < triCount; id++) {
    ...
    if (invert) {
      _tri.a.applyMatrix4(_matrix2);
      _tri.b.applyMatrix4(_matrix2);
      _tri.c.applyMatrix4(_matrix2);
    }
    const hitSide = getHitSide(_tri, bBVH, invert ? null : _matrix2);
    _actions.length = 0;
    _builders.length = 0;
    for (let o = 0, lo = operations.length; o < lo; o++) {
      const op = getOperationAction(operations[o], hitSide, invert);
    ...
    while (stack.length > 0) {
      ...
      if (_builders.length !== 0) {
        ...
        _tri.a.fromBufferAttribute(aPosition, i02);
        _tri.b.fromBufferAttribute(aPosition, i12);
        _tri.c.fromBufferAttribute(aPosition, i22);
        if (!isTriDegenerate(_tri)) {
          for (let k = 0, lk = _builders.length; k < lk; k++) {
            const builder = _builders[k];
            const action = _actions[k];
            const invertTri = action === INVERT_TRI;
            const invert2 = invertTri !== invertedGeometry;
            builder.appendIndexFromGeometry(a.geometry, _builderMatrix, _normalMatrix, groupIndex, i02, invert2);
            if (invert2) {
              builder.appendIndexFromGeometry(a.geometry, _builderMatrix, _normalMatrix, groupIndex, i22, invert2);
              builder.appendIndexFromGeometry(a.geometry, _builderMatrix, _normalMatrix, groupIndex, i12, invert2);
            } else {
              builder.appendIndexFromGeometry(a.geometry, _builderMatrix, _normalMatrix, groupIndex, i12, invert2);
              builder.appendIndexFromGeometry(a.geometry, _builderMatrix, _normalMatrix, groupIndex, i22, invert2);
            }
...
var GeometryBuilder = class {
  ...
  appendInterpolatedAttributeData(group, barycoord, index = null, invert = false) {
    ...
    if (index !== null && indexMap.has(index)) {
      ...
      for (const key in interpolatedFields) {
        ...
        getBarycoordValue(v0, v1, v2, barycoord, _vec42, isDirection, invertVector);
        pushItemSize(_vec42, itemSize, arr);
      }
    }
  }
  ...
  appendIndexFromGeometry(geometry, matrix, normalMatrix, group, index, invert = false) {
    ...
    if (index !== null && indexMap.has(index)) {
      ...
      for (const key in attributeData) {
        ...
        if (key === "position") {
          _vec32.fromBufferAttribute(attr, index).applyMatrix4(matrix);
          ...
          _vec32.fromBufferAttribute(attr, index).transformDirection(matrix);
          if (invert) {
            _vec32.multiplyScalar(-1);
          }
          arr.push(_vec32.x, _vec32.y, _vec32.z);
        } else {
          _vec42.fromBufferAttribute(attr, index);
          pushItemSize(_vec42, itemSize, arr);
        }
      }
    }
  ...
  buildGeometry(target, groupOrder) {
    ...
    for (let i = 0, l = Math.min(groupOrder.length, groupIndices.length); i < l; i++) {
      const { index: index2, materialIndex } = groupOrder[i];
      const { count } = groupIndices[index2];
      const buffer = groupIndices[index2].array.buffer;
      if (count !== 0) {
...
// node_modules/three-bvh-csg/src/core/operations/GeometryUtils.js
...
var Evaluator = class {
  ...
  evaluate(a, b, operations, targetBrushes = new Brush()) {
    ...
    const {
      ...
      useGroups,
      consolidateGroups,
      removeUnusedMaterials: removeUnusedMaterials2,
      debug
    } = this;
    ...
    if (!useGroups) {
      ...
      groups.sort((a2, b2) => a2.materialIndex - b2.materialIndex);
    }
    targetBrushes.forEach((brush, i) => {
  ...
  setHalfEdges(geometry, halfEdges) {
        ...
        if (halfEdges.disjointConnections) {
          for (let e = 0; e < 3; e++) {
            ...
            for (let i = 0; i < disjointTriIndices.length; i++) {
              ...
              getOverlapEdge(_tri1, e, _tri2, ei, _edge3);
    ...
    function addConnectionEdge(tri1, tri2, centerPoint) {
      ...
      if (straightEdges) {
        ...
        _vec23.addVectors(_center2, _projectedDir2);
        tri1.closestPointToPoint(_vec6, _finalPoint);
        tri2.closestPointToPoint(_vec23, _finalPoint2);
